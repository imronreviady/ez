		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
			...
			if(strpos($link, '&page=') == false) {
				$link = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://" . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] . "&page=%d";
			} else {
				$link = explode('&page=', $link);
				$link = $link[0] . "&page=%d";
			}

			$pagerContainer  = '<div class="btn-toolbar pagination-centered">';
			$pagerContainer .= '<div class="btn-group">';
			if( $totalpages != 0 )
			{
				if( $page == 1 )
				{
					$pagerContainer .= '<a href="#" class="btn btn-default disabled"> &#171; </a>';
				}
				else
				{
					$pagerContainer .= sprintf( '<a href="' . $link . '" class="btn btn-info"> &#171; </a>', $page - 1 );
				}
				$pagerContainer .= ' <span class="btn btn-default" style="curser: unset !important"> page <strong>' . $page . '</strong> from ' . $totalpages . '</span>';
				if( $page == $totalpages ) 
				{
					$pagerContainer .= '<a href="#" class="btn btn-default disabled"> &#187; </a>';
				}
