...
class Rel_role_permission_model extends CI_Model {
  ...
  function get()
  {
    return $this->db->get('ez_rel_role_permission')->result();
  }
  ...
  function get_by_role_id($role_id)
  {
    ...
    $this->db->from('ez_rel_role_permission');
    $this->db->join('ez_acl_permission', 'ez_rel_role_permission.permission_id = ez_acl_permission.id');
    $this->db->where("ez_rel_role_permission.role_id = $role_id AND ez_acl_permission.suspendedon IS NULL");
  ...
  function get_by_permission_id($permission_id)
  {
    ...
    $this->db->from('ez_rel_role_permission');
    $this->db->join('ez_acl_role', 'ez_rel_role_permission.role_id = ez_acl_role.id');
    $this->db->where("ez_rel_role_permission.permission_id = $permission_id AND ez_acl_role.suspendedon IS NULL");
  ...
  function update($role_id, $permission_id)
  {
    ...
    {
      $this->db->insert('ez_rel_role_permission', array('role_id' => $role_id, 'permission_id' => $permission_id));
    }
  }
  ...
  function update_batch($role_id, $permission_ids)
  {
    ...
    if( count($permission_ids) > 0)
    {
      ...
      $this->db->insert_batch('ez_rel_role_permission', $batch);
    }
  }
  ...
  {
    $this->db->delete('ez_rel_role_permission', array('role_id' => $role_id, 'permission_id' => $permission_id));
  }
  ...
  {
    $this->db->delete('ez_rel_role_permission', array('role_id' => $role_id));
  }
  ...
  {
    $this->db->delete('ez_rel_role_permission', array('permission_id' => $permission_id));
  }
}