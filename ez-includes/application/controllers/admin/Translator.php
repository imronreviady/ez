...
class Translator extends Admin_Controller {
		...
		if ( $this->_configured() ) {
			...
			if ( $this->validated && $this->input->post('ConfirmSaveLang') ) {
				if ( $this->_translation_saved( $result ) ) {
					$this->session->set_flashdata('message', ez_line('saved', ez_line('language')));
    ...
    public function newlang()
    {
    	...
    	if(is_demo() == FALSE) {
	        ...
	        $source = $this->langDir . '/' . $this->masterLang;

        	if($this->copyr($source, $path)) {
                $this->session->set_flashdata('message', ez_line('saved', ez_line('language')));        		
    ...
    public function delete($lang)
    {
    	...
    	if(is_demo() == FALSE) {
          	if(deleteDirectory($this->langDir . '/' . $lang)) {
            	$this->session->set_flashdata('message', ez_line('deleted', ez_line('language')));
            	redirect('admin/translator');
          	} else {
