	tinymce.PluginManager.add('mfnsc', function(editor, url) {
		...
		var base_url = url.replace('feast-content/themes/themeone/tinymce', '');
		...
		editor.addButton('mfnsc', {
			...
			menu : [ {
				...
				onclick : function() {
						...
						alert(base_url);
						var placeholderMarker = $(tinymce.getBody()).find('#cursorMarker');
						tinymce.selection.select(placeholderMarker.get(0) );
						...
						onclick:function(){
							editor.windowManager.open({
								...
								body:[
									{	
									    ...
									    'values': [
									        ...
									        {text: 'About with skills', value: 'about2'},
									        {text: 'Skills bar', value: 'about3'},
									        {text: 'Skills bar and Accordion', value: 'about4'},
									        {text: 'Services', value: 'services'},
									        {text: 'Video', value: 'videobg'},
								...
								onsubmit:function(b){
									...
									var content = url + '/shortcodes/sections/' + b.data.type + '.html';
									$.get(content, function(data) {

											var shortcode = ''
											switch(b.data.type) {
												case 'works':
													shortcode = "latest_works limit='6'";
													break; 
												case 'news':
													shortcode = "latest_posts limit='3'";
													break; 
											}
											...
											$.ajax({
												method: 'POST',
												url: base_url + '/Retrieve_Shortcode/retrieve',
												data: {shortcode: shortcode},
												success: function(d) {
													str = data.replace('[shortcode]', d);
													editor.insertContent(str + '<br><span id="cursorMarker"></span>');
													var placeholderMarker = $(tinymce.getBody()).find('#cursorMarker');
													tinymce.selection.select(placeholderMarker.get(0) );
													placeholderMarker.remove();
												}
						...
						onclick:function(){
							editor.windowManager.open({
								...
								onsubmit:function(b){
									...
									var shortcode = "slider alias=" + b.data.alias;
									/*
									$.ajax({ 
										method: 'POST',
										url: 'http://localhost/feast-cms/Retrieve_Shortcode/retrieve',
										data: {shortcode: shortcode},
										success: function(d) {
											//editor.insertContent(str+'<br><span id="cursorMarker"></span>');
											editor.insertContent(d+'<br><span id="cursorMarker"></span>');
											var placeholderMarker = $(tinymce.getBody()).find('#cursorMarker');
											tinymce.selection.select(placeholderMarker.get(0) );
											placeholderMarker.remove();
										}
						...
						onclick:function(){
							editor.windowManager.open({
								...
								onsubmit:function(b){
									...
									var shortcode = "latest_posts " + b.data.limit + " " + b.data.category;
									$.ajax({ 
										method: 'POST',
										url: base_url + '/Retrieve_Shortcode/retrieve',
										data: {shortcode: shortcode},
				...
				editor.windowManager.open({
					...
					body:[
						{	type: 'textbox',
						    name: 'percent',
						    label: 'Skill Percent',
						    value: $(node).attr('aria-valuenow')
						}
					],
					onsubmit:function(b){

						editor.dom.setAttrib(node, 'aria-valuenow', b.data.percent);
